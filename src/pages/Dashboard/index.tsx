    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-stone-50 to-slate-100">
      ...
      <aside className="fixed left-0 top-0 h-screen w-64 border-r border-slate-200 bg-white p-6">
        ...
        <nav className="space-y-2">
          ...
          <Link
            to="/my-resume"
            className="flex w-full items-center gap-3 rounded-lg px-4 py-3 text-slate-600 transition-colors hover:bg-slate-50"
          >
      ...
      <main className="ml-64 p-8">
        ...
        <div className="mx-auto mb-8 max-w-7xl">
          <h1 className="mb-2 text-4xl font-bold text-slate-900">My Resumes</h1>
          <p className="text-slate-600">
            Your first resume is free forever. Need more than one resume?{' '}
            <Link to="/pricing" className="font-medium text-slate-900 hover:underline">
              Upgrade your plan
            </Link>
          </p>
          ...
          <div className="relative mt-6">
            ...
            <input
              ...
              className="w-full max-w-md rounded-lg border border-slate-200 bg-white py-3 pl-12 pr-4 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-slate-900"
            />
          </div>
        </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              ...
              <button
                ...
                className="group flex aspect-[3/4] flex-col items-center justify-center gap-4 rounded-xl border-2 border-dashed border-slate-300 bg-white transition-all duration-200 hover:border-slate-400 hover:bg-slate-50"
              >
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-100 transition-colors group-hover:bg-slate-200">
              ...
              {filteredResumes.map((resume) => (
                <div
                  key={resume.id}
                  className="group relative aspect-[3/4] cursor-pointer overflow-hidden rounded-xl border-2 border-slate-200 bg-white transition-all duration-200 hover:border-slate-300 hover:shadow-lg"
                  onClick={() => handleEditResume(resume.id)}
                >
                  {/* Resume Preview/Thumbnail */}
                  <div className="absolute inset-0 bg-gradient-to-br from-slate-50 to-white p-6">
                    <div className="h-full w-full overflow-hidden rounded-lg border border-slate-200 bg-white p-4">
                      {/* Mock resume preview */}
                      <div className="space-y-3">
                        <div className="h-3 w-3/4 rounded bg-slate-900" />
                        <div className="h-2 w-1/2 rounded bg-slate-300" />
                        <div className="mt-4 h-2 w-2/3 rounded bg-slate-200" />
                  ...
                  {resume.metadata?.completenessScore && (
                    <div className="absolute left-4 top-4 z-10">
                      <div className="rounded-full border border-slate-200 bg-white/90 px-3 py-1 backdrop-blur">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 border-t border-slate-200 bg-white/95 p-4 backdrop-blur">
                    <div className="flex items-center justify-between">
                      <div className="min-w-0 flex-1">
                        <h3 className="truncate font-semibold text-slate-900">{resume.name}</h3>
                      ...
                      <div className="relative">
                        ...
                        {activeMenu === resume.id && (
                          <>
                            ...
                            />
                            <div className="absolute bottom-full right-0 z-30 mb-2 w-48 rounded-lg border border-slate-200 bg-white py-2 shadow-xl">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
